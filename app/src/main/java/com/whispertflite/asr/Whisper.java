...
//        this.mWhisperEngine = new WhisperEngineNative(context);
    ...
    public void loadModel(File modelPath, File vocabPath) {
        loadModel(modelPath.getAbsolutePath(), vocabPath.getAbsolutePath());
    }

    public void loadModel(String modelPath, String vocabPath) {
        try {
            mWhisperEngine.initialize(modelPath, vocabPath);
        } catch (IOException e) {
